...
- [ ] **Step 1.6 kernel 消费(正确 ordinary 序,A4)** — 按 `currentType()` 分派。**ORDINARY 顺序严格为**:
  ...
  ```
  TRACKED/SCHEDULE:读 `currentRecord()` 后 `advanceConsumer()+releaseCurrentSlot()`(早释放),再 run future/`timers.add`。指标 `completed/failed/cancelled` 覆盖**所有 accepted task**(含 ordinary),worker 单写者;每批 `gate.completeBatch(n)`。唤醒协议(spec §4.7):worker park 前 `parked.set(true)→admission acquire→双检`;生产者 `publish→gate.leave CAS→读取 parked` 后条件 unpark；只对未来 timer `parkNanos`,无 timer 无限 park,tombstone 也 unpark。
