...
- [x] **Step 4：实现显式 bounded/unbounded 表示**
...
- [x] **Step 4：实现 CancellationSource**
...
- [x] **Step 5：实现任务物理状态和 Future 唯一终态**
...
- [x] **Step 2：写 registry shutdownNow CAS 测试**
...
- [x] **Step 3：写两后端参数化 queue contract**
...
- [x] **Step 5：实现统一 reservation/publish 协议**
...
- [x] **Step 2：实现唯一 kernel 与 bounded facade**
...
- [x] **Step 3：写并通过 Executor/Future/阻塞保护测试**
...
- [x] **Step 6：写 JDK 21 shutdown 红灯测试**
...
- [x] **Step 3：只注入 UnboundedTaskQueue**
...
- [x] **Step 1：写选择、parent 和 owner gate 红灯测试**
...
- [x] **Step 1：添加 optional compile/test 依赖**
...
- [x] **Step 3：写 SmartLifecycle 真实 termination 测试**
...
- [x] **Step 2：加入可选 Commons profile**

POM 的 `commons-baseline` profile 只在显式 `-Pcommons-baseline` 时依赖 `cn.wjybxx.commons:commons-concurrent:2.0.0`，并通过 `build-helper-maven-plugin:3.6.1` 把 `src/commons/java` 加入该 profile 的编译源。运行前在参考仓安装当前 commit。Commons 的注解处理器位于独立 reactor，必须先安装 `java-apts`，再安装 `Commons-Concurrent` 及其依赖：

```bash
git -C /Users/sunke/dev/ai-project/commons rev-parse HEAD | grep -Fx '5c831c06afed35dd23b666c902e5ce212ff22487'
...
默认 reactor 不依赖本机参考仓；profile 同时校验固定 HEAD、tracked clean worktree 和三个
参考 JAR 的固定 SHA-256。缺少或解析到其它来源的同版本 artifact 时明确构建失败，不静默
跳过后又宣称已比较。
...
java -jar disruptor-benchmarks/target/benchmarks.jar com.sstlfsj.disruptor.benchmark.EventLoopBenchmark -wi 1 -i 2 -f 1
$MVN -pl disruptor-benchmarks -am -Pcommons-baseline clean package
java -jar disruptor-benchmarks/target/benchmarks.jar com.sstlfsj.disruptor.benchmark.CommonsEventLoopBenchmark -wi 1 -i 2 -f 1
```
